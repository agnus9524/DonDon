...
class ApiClient {
  ...
  async deleteTransaction(id: string): Promise<void> {
    ...
    if (!res.ok) throw new Error('전표 삭제 실패');
  }
  ...
  async importBankExcel(bank_account_id: string, rows?: any[]): Promise<{ count: number; transactions: Transaction[] }> {
    ...
    if (!res.ok) throw new Error('은행 거래내역 가져오기 실패');
    return res.json();
  }

  async getAdminUsersAndRoles(): Promise<{ users: User[]; roles: UserCompanyRole[]; companies: Company[] }> {
